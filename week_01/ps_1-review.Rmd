...
\uncover<4-5>{
\node[draw, text width = 6cm] at (3.25, -3) (dot2) {Intuitively, the dot product tells us how strongly $w$ and $v$ \underline{co-vary}.};
}
\uncover<5-5>{
...
If $\mathbf{A}^{-1}$ does not exist, $\mathbf{A}$ is called **singular**. How would we know $\mathbf{A}^{-1}$ singular? \pause
...
\end{bmatrix}
$$
\pause We can split these into two systems and solve each via Gauss-Jordan elmination:
$$
\begin{bmatrix}
2 & 3 \\
4 & 1
